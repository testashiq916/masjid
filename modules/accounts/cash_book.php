...
// Get cash account (account_code = 1001)
$cashAcc = $db->query("SELECT id, account_name, opening_balance FROM chart_of_accounts WHERE account_code = '1001' LIMIT 1")->fetch(PDO::FETCH_ASSOC);
$cashId  = (int)($cashAcc['id'] ?? 0);

$openingBalance = 0;
$transactions   = [];

if ($cashId) {
    // Opening balance = account opening_balance + all transactions before fromDate
    $openingIncome = $db->prepare("SELECT COALESCE(SUM(amount), 0) FROM receipts WHERE account_id = ? AND date < ? AND deleted_at IS NULL");
    $openingIncome->execute([$cashId, $fromDate]);

    $openingExpense = $db->prepare("SELECT COALESCE(SUM(amount), 0) FROM payments WHERE account_id = ? AND date < ? AND deleted_at IS NULL");
    $openingExpense->execute([$cashId, $fromDate]);

    // Also include journal entries before fromDate
    ...
    $openingJDebit->execute([$cashId, $fromDate]);
    ...
    $openingJCredit->execute([$cashId, $fromDate]);

    $openingBalance  = (float)($cashAcc['opening_balance'] ?? 0);
    ...
    $stmt = $db->prepare("
        SELECT 'receipt' AS txn_type, date, receipt_no AS ref_no,
               COALESCE(donor_name, 'N/A') AS party, ic.category_name AS description, amount, '' AS bank_ref
        FROM receipts r
        ...
        SELECT 'payment' AS txn_type, date, voucher_no AS ref_no,
               COALESCE(payee_name, 'N/A') AS party, ec.category_name AS description, amount, '' AS bank_ref
        FROM payments p
    ...
    ");
    $stmt->execute([$cashId, $fromDate, $toDate, $cashId, $fromDate, $toDate, $cashId, $fromDate, $toDate]);
...
<div class="content-area p-3 p-md-4" style="margin-top:56px;">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="mb-0"><i class="bi bi-cash me-2 text-primary"></i>Cash Book</h4>
            <small class="text-muted">
                <?= htmlspecialchars($cashAcc['account_name'] ?? 'Cash Account') ?> &mdash;
        ...
        <div class="card-body">
            <form method="GET" class="row g-2 align-items-end">
                ...
                <div class="col-md-4 d-flex gap-2">
                    ...
                    <a href="<?= BASE_PATH ?>/modules/accounts/cash_book.php" class="btn btn-outline-secondary" title="Reset"><i class="bi bi-arrow-counterclockwise"></i></a>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="alert alert-warning">
        ...
        Cash account (account_code = '1001') not found. Please create it in Chart of Accounts.
    </div>
    <?php else: ?>
    ...
    <div class="row g-3 mb-4">
        ...
        <div class="col-md-3">
            <div class="card border-success">
                <div class="card-body py-2">
                    <div class="text-muted small">Total Receipts (Dr)</div>
        ...
        <div class="col-md-3">
            <div class="card border-danger">
                <div class="card-body py-2">
                    <div class="text-muted small">Total Payments (Cr)</div>
        ...
        <div class="col-md-3">
            <div class="card border-primary">
                <div class="card-body py-2">
                    ...
                    <div class="fw-bold fs-5 <?= $closingBalance < 0 ? 'text-danger' : 'text-primary' ?>"><?= formatCurrency($closingBalance) ?></div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card shadow-sm">
        <div class="card-header bg-white fw-semibold d-flex justify-content-between align-items-center">
            <span><i class="bi bi-cash me-2 text-primary"></i>Cash Book Statement</span>
                ...
                <table class="table table-hover table-bordered align-middle mb-0">
                    <thead class="table-primary">
                        <tr>
                            ...
                            <th>Particulars</th>
                            <th>Ref. No.</th>
                            <th class="text-end text-success">Receipts (Dr)</th>
                            <th class="text-end text-danger">Payments (Cr)</th>
                    ...
                    <tbody>
                        ...
                        <tr class="table-secondary fw-bold">
                            <td colspan="3" class="text-end">Period Total</td>
                        ...
                        <tr class="table-warning fw-bold">
                            <td colspan="3" class="text-end">Closing Balance c/f</td>
                            <td></td>
                            <td></td>
