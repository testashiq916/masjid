...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save') {
        ...
        try {
            if ($id > 0) {
                ...
                logAudit('update', 'expense_categories', $id);
                ...
                logAudit('create', 'expense_categories', $newId);
    ...
    if ($action === 'delete') {
        ...
        if ($id > 0) {
            // Check if in use
            $cnt = db()->prepare("SELECT COUNT(*) FROM payments WHERE category_id = ? AND deleted_at IS NULL");
            $cnt->execute([$id]);
            if ((int)$cnt->fetchColumn() > 0) {
                echo json_encode(['success' => false, 'message' => 'Cannot delete: category is used in existing payments.']);
                exit;
            }
            db()->prepare("UPDATE expense_categories SET deleted_at = NOW() WHERE id = ?")->execute([$id]);
            logAudit('delete', 'expense_categories', $id);
    ...
    if ($action === 'get') {
        $id   = (int)($_POST['id'] ?? 0);
        $stmt = db()->prepare("SELECT * FROM expense_categories WHERE id = ? AND deleted_at IS NULL");
        $stmt->execute([$id]);
...
$categories = db()->query("
    SELECT ec.*, coa.account_name,
           (SELECT COUNT(*) FROM payments p WHERE p.category_id = ec.id AND p.deleted_at IS NULL) AS payment_count,
           (SELECT COALESCE(SUM(p.amount),0) FROM payments p WHERE p.category_id = ec.id AND p.deleted_at IS NULL) AS total_amount
...
$expenseAccounts = db()->query("
    ...
    WHERE account_type IN ('Expense', 'Expenses')
...
<div class="content-area p-3 p-md-4" style="margin-top:56px;">
    ...
    <div class="card shadow-sm">
        <div class="card-header bg-white d-flex justify-content-between align-items-center">
            <span class="fw-semibold"><i class="bi bi-tags me-1 text-danger"></i> Expense Categories</span>
            <button class="btn btn-danger btn-sm" onclick="openCatModal(0)">
                <i class="bi bi-plus-circle me-1"></i> Add Category
            </button>
        </div>
        <div class="card-body p-0">
            <div class="table-responsive">
                <table id="expCatTable" class="table table-hover table-striped mb-0 align-middle" style="width:100%;">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-muted"><?= htmlspecialchars($cat['account_name'] ?? '-') ?></td>
                            <td class="text-muted small"><?= htmlspecialchars($cat['description'] ?? '') ?></td>
                            <td class="text-center">
                                <span class="badge bg-warning text-dark"><?= (int)$cat['payment_count'] ?></span>
                            </td>
        ...
        <div class="modal-content">
            <div class="modal-header bg-danger text-white">
                <h5 class="modal-title" id="catModalLabel">
                    <i class="bi bi-tag me-1"></i> <span id="catModalTitle">Add Expense Category</span>
                </h5>
                ...
                <form id="catForm" novalidate>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="cat_name" name="category_name"
                               placeholder="e.g. Electricity, Maintenance, Salary" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-semibold">Linked Expense Account (COA)</label>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-danger" id="saveCatBtn">
...
$(function () {
    $('#expCatTable').DataTable({
        pageLength: 25,
        order: [[1, 'asc']],
        language: { emptyTable: 'No expense categories found.' }
    });
});
...
function deleteCat(id, name) {
    if (!confirm('Delete category "' + name + '"?\nThis will fail if there are existing payments linked to it.')) return;
    $.post(window.location.pathname, {
        action: 'delete', id: id,
        csrf_token: $('input[name=csrf_token]').first().val()
    }, function (res) {
